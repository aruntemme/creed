...
export async function proxy(request: NextRequest) {
  ...
  // pathname here lets the root layout skip expensive data fan-out for
  // marketing routes that never read user state.
  requestHeaders.set("x-pathname", request.nextUrl.pathname);

  // Auth.js uses a stateless JWT session cookie read directly by server code,
  // so there's no middleware token-refresh step to run (the Supabase
  // @supabase/ssr cookie refresh that used to live here is no longer needed).
